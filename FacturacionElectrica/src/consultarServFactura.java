 ...
 */
public class consultarServFactura extends javax.swing.JDialog {

    /**
     * Creates new form generarFactura
     */
    public consultarServFactura(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        entradaNFactura.requestFocus();
    }
    ...
    private void initComponents() {
        ...
        entradaNFactura.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                entradaNFacturaActionPerformed(evt);
            }
        });
    ...
    private String obtener_cod_medidor(String codigo_fact) {
        String cod_me = "";
        conexión prueba = new conexión();
        try {
            try {
                prueba.conectar();
                ArrayList a = prueba.impresion("SELECT LEC_FACT_PAGADA.COD_MEDIDOR FROM LEC_FACT_PAGADA WHERE LEC_FACT_PAGADA.NUM_FAC_SERV=" + codigo_fact);
                if (a.size() > 1) {
                    String resultados[] = (String[]) a.get(1);
                    cod_me = resultados[0];
                } else {
    ...
    private void entradaNFacturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_entradaNFacturaActionPerformed
        ...
        String cod_medidor = obtener_cod_medidor(entradaNFactura.getText());
        if (cod_medidor.length() > 1) {
            conexión prueba = new conexión();
            try {
                try {
                    prueba.conectar();
                    ArrayList a = prueba.impresion("select \"MEDIDOR\".\"COD_MEDIDOR\" as \"Codigo Medidor\", "
                            ...
                            + "and 	 \"MEDIDOR\".\"COD_MEDIDOR\" = " + "'" + cod_medidor + "'");
    ...
    }//GEN-LAST:event_entradaNFacturaActionPerformed
